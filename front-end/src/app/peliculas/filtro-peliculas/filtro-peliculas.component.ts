...
export class FiltroPeliculasComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.generosService.obtenerTodos().subscribe({
      next: (generos) => {
        ...
        //Indico que en cada cambio del formulario se hará una acción
        this.form.valueChanges.subscribe((valores) => {
          this.buscarPeliculas(valores),
          this.escribirParametrosBusquedaEnURL()
        }); //Un observable nos permite subscribirnos a una secuencia de eventos que se disparan en el tiempo
      },
    });
  }
  ...
  private escribirParametrosBusquedaEnURL() {
    ...
    this.location.replaceState('peliculas/buscar', queryStrings.join('&'));//Location -> permite escribir la URL como desees
  }
